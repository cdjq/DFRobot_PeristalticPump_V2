/*!
 * @file timerPump.ino
 * @brief Run the pump by fixed time (call once)
 * @copyright Copyright (c) 2026 DFRobot Co.Ltd (http://www.dfrobot.com)
 * @license The MIT License (MIT)
 * @author JiaLi(zhixin.liu@dfrobot.com)
 * @version V1.0
 * @date 2026-02-10
 * @url https://github.com/cdjq/DFRobot_PeristalticPump_V2
 */

#include "DFRobot_PeristalticPump_V2.h"

#if defined(ESP32)
int servoPin = 17;
#else
int servoPin = 9;
#endif

DFRobot_PeristalticPump_V2 pump(servoPin);

uint8_t timePumpFlag = 1;
float   volume       = 0.0f;

void onTimerPumpCallback(float volume, bool finished)
{
  if (finished) {
    Serial.print(F("Final volume(ml): "));
    Serial.println(volume, 4);
  } else {
    Serial.print(F("Current volume(ml): "));
    Serial.println(volume, 4);
  }
}

void setup()
{
  Serial.begin(115200);
  pump.begin();
  pump.setTimerPumpCallback(onTimerPumpCallback, 50);

  Serial.println(F("timerPump demo (call once)."));
  Serial.println(F("Run calPump.ino first to store flowRate in EEPROM."));
}

void loop()
{
  // Update pump status; this must be called repeatedly in loop().
  pump.updatePumpStatus();

  /**
   * Note:
   * 1. Use timerPump() to run the pump for a fixed duration.
   * 2. Parameter 1: run time in milliseconds.
   * 3. Parameter 2: pointer to a float for the precalculated pumped volume (ml).
   * 4. Return true: task started successfully.
   * 5. Return false: start failed.
   *    - A task may already be running. Wait until it finishes, or call stopPump().
   *    - Calibration data may be invalid. Run calPump.ino to recalibrate flow rate.
   */
  if (timePumpFlag) {
    timePumpFlag = 0;
    Serial.println(F("Start: timerPump 5000ms"));
    if (pump.timerPump(5000, &volume)) {
      Serial.print(F("Precalculated volume(ml): "));
      Serial.println(volume, 4);
    } else {
      Serial.println(F("timerPump start failed."));
    }
  }
}
